codeunit 50004 "Whse.-Shipment ReleaseNew"
{
    trigger OnRun()
    begin
    end;
    ...
    begin
        with WhseShptHeader do begin
            ...
            if not WhseShptLine.Find('-') then Error(Text000, TableCaption, "No.");
            ...
            repeat
                ...
                if Location."Directed Put-away and Pick" then WhseShptLine.TestField("Zone Code");
                if Location."Bin Mandatory" then begin
                    ...
                    if WhseShptLine."Assemble to Order" then begin
                        ...
                        if AsmLine.FindSet then
                            repeat
                                if AsmLine.CalcQtyToPickBase > 0 then AsmLine.TestField("Bin Code");
            ...
            if not WhsePickRqst.IsEmpty then WhsePickRqst.DeleteAll(true);
            Commit;
        end;
    ...
    begin
        with WhseShptHeader do begin
            ...
            if not PickWkshLine.IsEmpty then Error(Text001);
            ...
            if not WhseActivLine.IsEmpty then Error(Text002);
            WhsePickRqst.SetRange("Document Type", WhsePickRqst."Document Type"::Shipment);
            WhsePickRqst.SetRange("Document No.", "No.");
            WhsePickRqst.SetRange(Status, Status::Released);
            if not WhsePickRqst.IsEmpty then WhsePickRqst.ModifyAll(Status, WhsePickRqst.Status::Open);
            Status := Status::Open;
            Modify;
        end;
    ...
    var
        ...
        Location: Record Location;
    begin
        with WhseShptHeader do if Location.RequirePicking("Location Code") then begin
                ...
                if not WhsePickRqst.Insert then WhsePickRqst.Modify;
            end;
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterRelease(var WarehouseShipmentHeader: Record "Warehouse Shipment Header";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterTestWhseShptLine(var WarehouseShipmentHeader: Record "Warehouse Shipment Header";
    var WarehouseShipmentLine: Record "Warehouse Shipment Line")
    begin
    end;
