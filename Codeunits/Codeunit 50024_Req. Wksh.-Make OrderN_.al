codeunit 50024 "Req. Wksh.-Make OrderN"
{
    ...
    trigger OnRun()
    begin
        if PlanningResiliency then LockTable;
    ...
    begin
        ...
        with ReqLine do begin
            ...
            if not PlanningResiliency then LockTable;
            if "Planning Line Origin" <> "Planning Line Origin"::"Order Planning" then ReqTemplate.Get("Worksheet Template Name");
            ...
            if not Find('=><') then begin
                "Line No." := 0;
                if not SuppressCommit then Commit;
                exit;
            end;
            if ReqTemplate.Recurring then
                Window.Open(Text000 + Text001 + Text002 + Text003 + Text004)
            else
                Window.Open(Text000 + Text001 + Text002 + Text003 + Text005);
            ...
            Clear(PurchOrderHeader);
            SetPurchOrderHeader;
            SetCurrentKey("Worksheet Template Name", "Journal Batch Name", "Vendor No.", "Sell-to Customer No.", "Ship-to Code", "Order Address Code", "Currency Code", "Ref. Order Type", "Ref. Order Status", "Ref. Order No.", "Location Code", "Transfer-from Code", "Purchasing Code");
            if Find('-') then
                repeat
                ...
                until Next = 0;
            if PrintPurchOrders then PrintTransOrder(TransHeader);
            if PurchOrderHeader."Buy-from Vendor No." <> '' then FinalizeOrderHeader(PurchOrderHeader, ReqLine);
            if PrevChangedDocOrderNo <> '' then PrintChangedDocument(PrevChangedDocOrderType, PrevChangedDocOrderNo);
                ...
                if not ReqTemplate.Recurring then begin
                    ...
                    if Find('-') then
                        repeat
                            ...
                            if not TempFailedReqLine.Find then Delete(true);
                        until Next = 0;
    ...
    begin
        with ReqLine2 do begin
            if ("No." <> '') or ("Vendor No." <> '') or (Quantity <> 0) then begin
                TestField("No.");
                if "Action Message" <> "Action Message"::Cancel then TestField(Quantity);
                if ("Action Message" = "Action Message"::" ") or ("Action Message" = "Action Message"::New) then
                    if "Replenishment System" = "Replenishment System"::Purchase then begin
                        if "Planning Line Origin" = "Planning Line Origin"::"Order Planning" then TestField("Supply From");
                        TestField("Vendor No.")
                    end
                    else
            ...
            if not DimMgt.CheckDimIDComb("Dimension Set ID") then Error(Text008, TableCaption, "Worksheet Template Name", "Journal Batch Name", "Line No.", DimMgt.GetDimCombErr);
            TableID[1] := DimMgt.TypeToTableID3(Type);
            No[1] := "No.";
            if not DimMgt.CheckDimValuePosting(TableID, No, "Dimension Set ID") then
                if "Line No." <> 0 then
                    Error(Text009, TableCaption, "Worksheet Template Name", "Journal Batch Name", "Line No.", DimMgt.GetDimValuePostingErr)
                else
                    Error(DimMgt.GetDimValuePostingErr);
            if SalesLine.Get(SalesLine."Document Type"::Order, "Sales Order No.", "Sales Order Line No.") and (SalesLine."Unit of Measure Code" <> "Unit of Measure Code") then if SalesLine."Drop Shipment" or (PurchasingCode.Get("Purchasing Code") and PurchasingCode."Drop Shipment") then FieldError("Unit of Measure Code", StrSubstNo(Text010, SalesLine.FieldCaption("Unit of Measure Code"), SalesLine."Document No.", SalesLine."Line No."));
            if Purchasing.Get("Purchasing Code") then
                if Purchasing."Drop Shipment" or Purchasing."Special Order" then begin
                    ...
                    if (Purchasing."Drop Shipment" <> SalesLine."Drop Shipment") or (Purchasing."Special Order" <> SalesLine."Special Order") then FieldError("Purchasing Code", StrSubstNo(Text010, SalesLine.FieldCaption("Purchasing Code"), SalesLine."Document No.", SalesLine."Line No."));
                end;
        end;
    ...
    begin
        ...
        with ReqLine do case "Replenishment System" of
                            "Replenishment System"::Transfer:
                                case "Action Message" of
                                    ...
                                    "Action Message"::"Change Qty.", "Action Message"::Reschedule, "Action Message"::"Resched. & Chg. Qty.":
                                        begin
                                            if (PrevChangedDocOrderNo <> '') and (("Ref. Order Type" <> PrevChangedDocOrderType) or ("Ref. Order No." <> PrevChangedDocOrderNo)) then PrintChangedDocument(PrevChangedDocOrderType, PrevChangedDocOrderNo);
                            ...
                            "Replenishment System"::Purchase, "Replenishment System"::"Prod. Order":
                                case "Action Message" of
                                    ...
                                    "Action Message"::New, "Action Message"::" ":
                                        begin
                                            if (PurchOrderHeader."Buy-from Vendor No." <> '') and CheckInsertFinalizePurchaseOrderHeader(ReqLine, PurchOrderHeader, false) then begin
                                                ...
                                                if PurchOrderLine.Find('-') then
                                                    repeat
                                                        SalesOrderLine.Get(SalesOrderLine."Document Type"::Order, PurchOrderLine."Special Order Sales No.", PurchOrderLine."Special Order Sales Line No.");
    ...
    begin
        with ReqLine do begin
            ...
            ReqWkshMakeOrders.SetTryParam(ReqTemplate, LineCount, NextLineNo, PrevPurchCode, PrevShipToCode, PrevLocationCode, OrderCounter, OrderLineCounter, TempFailedReqLine, TempDocumentEntry);
            ReqWkshMakeOrders.SetSuppressCommit(SuppressCommit);
            if ReqWkshMakeOrders.Run(ReqLine) then begin
                ReqWkshMakeOrders.GetTryParam(PurchOrderHeader, LineCount, NextLineNo, PrevPurchCode, PrevShipToCode, PrevLocationCode, OrderCounter, OrderLineCounter);
    ...
    local procedure InitPurchOrderLine(var PurchOrderLine: Record "Purchase Line";
    PurchOrderHeader: Record "Purchase Header";
    ...
    begin
        with RequisitionLine do begin
            ...
            //Ravi Added New Flds
            PurchOrderLine."Web Order Id" := WebOrderNo;
            PurchOrderLine.GSOrderItemUID := GSOrderItemUID;
            PurchOrderLine.CreatedOn := Today;
            PurchOrderLine."Order Status" := OrderStatus;

            IF orderTrack.get(GSOrderItemUID) then begin
                orderTrack."PO No." := PurchOrderHeader."No.";
                orderTrack.Modify();
            end;
    ...
    local procedure InsertPurchOrderLine(var ReqLine2: Record "Requisition Line";
    ...
    begin
        with ReqLine2 do begin
            if ("No." = '') or ("Vendor No." = '') or (Quantity = 0) then exit;
            TestField(ReqLine2."Direct Unit Cost");
            OrderTracking.GET(ReqLine2.GSOrderItemUID);
            IF OrderTracking."Order Item Status" = OrderTracking."Order Item Status"::Canceled then exit;
            ...
            LineCount := LineCount + 1;
            if not PlanningResiliency then Window.Update(4, LineCount);
            ...
            PurchOrderLine."Dimension Set ID" := DimMgt.GetCombinedDimensionSetID(DimensionSetIDArr, PurchOrderLine."Shortcut Dimension 1 Code", PurchOrderLine."Shortcut Dimension 2 Code");
            ...
            if Reserve then ReserveBindingOrderToPurch(PurchOrderLine, ReqLine2);
            if PurchOrderLine."Drop Shipment" or PurchOrderLine."Special Order" then begin
                ...
                if not PurchOrderLine."Special Order" then TestField("Ship-to Code", SalesOrderHeader."Ship-to Code");
                ...
                if SalesOrderLine."Purch. Order Line No." <> 0 then Error(Text006, SalesOrderLine."No.", SalesOrderLine."Document No.", SalesOrderLine."Purchase Order No.");
                if SalesOrderLine."Special Order Purchase No." <> '' then Error(Text006, SalesOrderLine."No.", SalesOrderLine."Document No.", SalesOrderLine."Special Order Purchase No.");
                if not PurchOrderLine."Special Order" then TestField("Sell-to Customer No.", SalesOrderLine."Sell-to Customer No.");
                TestField(Type, SalesOrderLine.Type);
                TestField(Quantity, Round(SalesOrderLine."Outstanding Quantity" * SalesOrderLine."Qty. per Unit of Measure" / "Qty. per Unit of Measure", 0.00001));
            ...
            if TransferExtendedText.PurchCheckIfAnyExtText(PurchOrderLine, false) then begin
                ...
                if PurchOrderLine2.FindLast then NextLineNo := PurchOrderLine2."Line No.";
            end;
        end;
    ...
    begin
        with ReqLine2 do begin
            OrderCounter := OrderCounter + 1;
            if not PlanningResiliency then Window.Update(3, OrderCounter);
            PurchSetup.Get;
            ...
            if "Order Address Code" <> '' then PurchOrderHeader.Validate("Order Address Code", "Order Address Code");
            if "Sell-to Customer No." <> '' then PurchOrderHeader.Validate("Sell-to Customer No.", "Sell-to Customer No.");
            PurchOrderHeader.Validate("Currency Code", "Currency Code");
            if PurchasingCode.Get("Purchasing Code") then if PurchasingCode."Special Order" then SpecialOrder := true;
            if not SpecialOrder then begin
                ...
                if Vendor.Get(PurchOrderHeader."Buy-from Vendor No.") then PurchOrderHeader.Validate("Shipment Method Code", Vendor."Shipment Method Code");
            end;
            ...
            if SpecialOrder then if Vendor.Get(PurchOrderHeader."Buy-from Vendor No.") then PurchOrderHeader."Shipment Method Code" := Vendor."Shipment Method Code";
    ...
    local procedure FinalizeOrderHeader(PurchOrderHeader: Record "Purchase Header";
    ...
    begin
        if ReqTemplate.Recurring then begin
            ...
            repeat
                ...
                if not PlanningResiliency then Window.Update(5, OrderLineCounter);
    ...
    begin
        with ReqLine2 do if "No." <> '' then
                if ReqTemplate.Recurring then begin
                    TestField("Recurring Method");
                    TestField("Recurring Frequency");
                    if "Recurring Method" = "Recurring Method"::Variable then TestField(Quantity);
                end
                else begin
    ...
    begin
        with ReqLine2 do if ("No." <> '') and ("Recurring Method" <> 0) and ("Order Date" <> 0D) then begin
                ...
                Description := DelChr(PadStr(StrSubstNo(Description, Day, Week, Month, MonthText, AccountingPeriod.Name), MaxStrLen(Description)), '>');
                Modify;
            end;
    end;

    local procedure ReserveBindingOrderToPurch(var PurchLine: Record "Purchase Line";
    var ReqLine: Record "Requisition Line")
    var
    ...
    procedure SetTryParam(TryReqTemplate: Record "Req. Wksh. Template";
    ...
    var TryFailedReqLine: Record "Requisition Line";
    var TempDocumentEntryNew: Record "Document Entry" temporary)
    begin
    ...
    procedure GetTryParam(var TryPurchOrderHeader: Record "Purchase Header";
    var TryLineCount: Integer;
    var TryNextLineNo: Integer;
    var TryPrevPurchCode: Code[10];
    var TryPrevShipToCode: Code[10];
    var TryPrevLocationCode: Code[10];
    var TryOrderCounter: Integer;
    var TryOrderLineCounter: Integer)
    begin
    ...
    local procedure PrintChangedDocument(OrderType: Option;
    var OrderNo: Code[20])
    var
    ...
    local procedure CheckAddressDetails(SalesOrderNo: Code[20];
    SalesLineNo: Integer;
    UpdateAddressDetails: Boolean) Result: Boolean
    var
        SalesLine: Record "Sales Line";
        Purchasing: Record Purchasing;
    begin
            ...
            if Purchasing.Get(SalesLine."Purchasing Code") then
                case true of
                    Purchasing."Drop Shipment":
                        Result := not CheckDropShptAddressDetails(SalesOrderNo, UpdateAddressDetails);
                    Purchasing."Special Order":
                        Result := not CheckSpecOrderAddressDetails(SalesLine."Location Code");
                end;
    end;
    ...
    begin
        InventorySetup.Get;
        if InventorySetup."Location Mandatory" then RequisitionLine.TestField("Location Code");
    end;

    local procedure CheckInsertFinalizePurchaseOrderHeader(RequisitionLine: Record "Requisition Line";
    var PurchOrderHeader: Record "Purchase Header";
    UpdateAddressDetails: Boolean) Result: Boolean
    begin
        with RequisitionLine do Result := (PurchOrderHeader."Buy-from Vendor No." <> "Vendor No.") or // (PurchOrderHeader."Sell-to Customer No." <> "Sell-to Customer No.") or
     (PrevShipToCode <> "Ship-to Code") or (PurchOrderHeader."Order Address Code" <> "Order Address Code") or (PurchOrderHeader."Currency Code" <> "Currency Code") or (PrevPurchCode <> "Purchasing Code") or (PrevLocationCode <> "Location Code") or CheckAddressDetails("Sales Order No.", "Sales Order Line No.", UpdateAddressDetails);
    end;

    local procedure CheckDropShptAddressDetails(SalesNo: Code[20];
    UpdateAddressDetails: Boolean): Boolean
    var
        SalesHeader: Record "Sales Header";
    ...
    begin
        ...
        if NameAddressDetails = DropShptNameAddressDetails then exit(true);
        if UpdateAddressDetails then NameAddressDetails := DropShptNameAddressDetails;
        exit(false);
    end;
    ...
    begin
        if Location.Get(LocationCode) then
            SpecOrderNameAddressDetails := Location.Name + Location."Name 2" + Location.Address + Location."Address 2" + Location."Post Code" + Location.City + Location.Contact
        else begin
            CompanyInfo.Get;
    ...
    begin
        if PurchOrderLine."Drop Shipment" then exit((ReqLine."Sales Order No." = PurchOrderLine."Sales Order No.") and (ReqLine."Sales Order Line No." = PurchOrderLine."Sales Order Line No."));
        if PurchOrderLine."Special Order" then exit((ReqLine."Sales Order No." = PurchOrderLine."Special Order Sales No.") and (ReqLine."Sales Order Line No." = PurchOrderLine."Special Order Sales Line No."));
        exit(true);
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCarryOutReqLineAction(var RequisitionLine: Record "Requisition Line";
    var Failed: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforePurchOrderLineInsert(var PurchOrderHeader: Record "Purchase Header";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePurchOrderLineValidateNo(var PurchOrderLine: Record "Purchase Line";
    var RequisitionLine: Record "Requisition Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterPurchOrderLineInsert(var PurchOrderLine: Record "Purchase Line";
    var RequisitionLine: Record "Requisition Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCarryOutReqLineAction(var RequisitionLine: Record "Requisition Line";
    var PurchaseHeader: Record "Purchase Header";
    CommitIsSuppressed: Boolean;
    var OrderCounter: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterReserveBindingOrderToPurch(var PurchaseLine: Record "Purchase Line";
    var ReqLine: Record "Requisition Line";
    ReservQty: Decimal;
    ReservQtyBase: Decimal;
    CommitIsSuppressed: Boolean)
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterSet(NewPurchOrderHeader: Record "Purchase Header";
    CommitIsSuppressed: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCheckReqWkshLine(var RequisitionLine: Record "Requisition Line";
    CommitIsSuppressed: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInsertPurchOrderHeader(var RequisitionLine: Record "Requisition Line";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterInsertPurchOrderLine(var PurchOrderLine: Record "Purchase Line";
    NextLineNo: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterFinalizeOrderHeader(PurchHeader: Record "Purchase Header";
    var ReqLine: Record "Requisition Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnCheckNewNameNeccessary(RequisitionWkshName: Record "Requisition Wksh. Name";
    var NewReqWkshName: Boolean)
    begin
    end;
    ...
    local procedure CheckCustomGSLogic(var ReqLines: Record "Requisition Line")
    var
        orderTrack: Record "Order Tracking Table";
    begin
        ReqLines.TestField("Direct Unit Cost");
        orderTrack.Get(ReqLines.GSOrderItemUID); //RAVI Code Comment to avoid checking OrderTrack table 04Aug 2015
        if not (orderTrack."Order Item Status" <> orderTrack."Order Item Status"::Released) then //Ravi 11th June 2015 to check Released Status for PO
            exit;
    end;
}
