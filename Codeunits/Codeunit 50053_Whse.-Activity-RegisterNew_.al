codeunit 50053 "Whse.-Activity-RegisterNew"
{
    Permissions = TableData "Registered Whse. Activity Hdr." = i, TableData "Registered Whse. Activity Line" = i, TableData "Whse. Item Tracking Line" = rim, TableData "Warehouse Journal Batch" = imd, TableData "Posted Whse. Receipt Header" = m, TableData "Posted Whse. Receipt Line" = m, TableData "Registered Invt. Movement Hdr." = i, TableData "Registered Invt. Movement Line" = i;
    ...
    begin
        ...
        with WhseActivHeader do begin
            ...
            if WhseActivLine.IsEmpty then Error(Text003);
            ...
            if WhseActivLine.Find('-') then begin
                ...
                repeat
                    ...
                    //UpdateCrossDockPickwrtOrdItem(WhseActivLine); //Ravi Added to update Bin On Picks

                    //UpdateBack2BackOrder(WhseActivLine);
            ...
            WhseActivLine.SetCurrentKey("Activity Type", "No.", "Whse. Document Type", "Whse. Document No.");
            if WhseActivLine.Find('-') then
                repeat
                    //Ravi New change

                    Locate.Get(WhseActivLine."Location Code");
                    Locate.TestField(CrossDockZone);

                    IF not ssetup.PutawayB2BAllocation then begin
                        if (WhseActivLine."Source Type" = 39) and (WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Put-away") and (WhseActivLine."Action Type" = WhseActivLine."Action Type"::Place) and (WhseActivLine."Bin Code" <> locate."Receipt Bin Code") and (whseActivLine."Zone Code" = locate.CrossDockZone) then begin

                            IF WhseActivLine.GSOrderItemUID <> '' THEN
                                UpdateCrossDockPickwrtOrdItem(WhseActivLine);
                        end;
                    end;


                    CopyWhseActivityLineToReservBuf(TempWhseActivLineToReserve, WhseActivLine);
                    if Type <> Type::Movement then UpdateWhseSourceDocLine(WhseActivLine);
                    ...
                    if WhseActivLine."Qty. Outstanding" = WhseActivLine."Qty. to Handle" then begin
                        ...
                        if not SkipDelete then WhseActivLine.Delete;
                    end
                    else begin
                        ...
                        if HideDialog then WhseActivLine.Validate("Qty. to Handle", 0);
                    ...
                    if LastLine or (OldWhseActivLine."Whse. Document Type" <> WhseActivLine."Whse. Document Type") or (OldWhseActivLine."Whse. Document No." <> WhseActivLine."Whse. Document No.") or (OldWhseActivLine."Action Type" <> WhseActivLine."Action Type") then UpdateWhseDocHeader(OldWhseActivLine);
            ...
            if Location."Bin Mandatory" then begin
                ...
                if WhseActivLine.Find('-') then
                    repeat
                        if ((LineCount = 1) and ((OldWhseActivLine."Whse. Document Type" <> WhseActivLine."Whse. Document Type") or (OldWhseActivLine."Whse. Document No." <> WhseActivLine."Whse. Document No."))) then begin
                            LineCount := 0;
                        ...
                        LineCount := LineCount + 1;
                    until WhseActivLine.Next = 0;
                if LineCount = 1 then OldWhseActivLine.Delete;
            end;
            ...
            if not WhseActivLine.Find('-') then begin
                ...
                if not SkipDelete then Delete(true);
            end
            else begin
                "Last Registering No." := "Registering No.";
                "Registering No." := '';
                Modify;
                if not HideDialog then WhseActivLine.AutofillQtyToHandle(WhseActivLine);
                OnAfterAutofillQtyToHandle(WhseActivLine);
            end;
            if not HideDialog then Window.Close;
    ...
    var
        ...
        whsTemplateCode: code[20];
        whsBatchcode: Code[20];
        lineNos: Integer;
        ssetup: Record "Sales & Receivables Setup";
    begin
        ssetup.get;
        ...
        with WhseActivLine do begin
            ...
            IF ssetup.EnableUserTemplates THEN BEGIN
                IF WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Put-away" Then
                    whsTemplateCode := UserSetup.PutAwayTemplates
                else
                    whsTemplateCode := UserSetup.PickTemplates;

                WhseJnlLine.Reset;
                WhseJnlLine.SetRange("Journal Template Name", whsTemplateCode);
                WhseJnlLine.Setrange("Journal Batch Name", 'DEFAULT');
                IF WhseJnlLine.FindLast THEN
                    lineNos := WhseJnlLine."Line No."
                else
                    lineNos := 0;
            ...
            If ssetup.EnableUserTemplates then begin
                WhseJnlLine.VALIDATE("Journal Template Name", whsTemplateCode);
                WhseJnlLine.Validate("Journal Batch Name", 'DEFAULT');
                WhseJnlLine."Line No." := lineNos;
            end;
            ...
            if Location."Directed Put-away and Pick" then begin
                ...
                WhseJnlLine.Cubage := Abs(WhseJnlLine.Quantity) * ItemUnitOfMeasure.Cubage;
                WhseJnlLine.Weight := Abs(WhseJnlLine.Quantity) * ItemUnitOfMeasure.Weight;
            end
            else begin
            ...
            case "Activity Type" of
                ...
                "Activity Type"::"Invt. Put-away", "Activity Type"::"Invt. Pick", "Activity Type"::"Invt. Movement":
                    WhseJnlLine."Whse. Document Type" := WhseJnlLine."Whse. Document Type"::" ";
            end;
            if "Serial No." <> '' then TestField("Qty. per Unit of Measure", 1);
    ...
    var
        WhseActLinesPut: Record "Warehouse Activity Line";
        locate: Record Location;
    ...
    begin
        ...
        if WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Invt. Movement" then begin
            ...
            Locate.TestField(CrossDockZone);
            //some rnd ravi
            IF salesSetup.PutawayB2BAllocation then begin
                if (WhseActivLine."Source Type" = 39) and (WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Put-away") and (WhseActivLine."Action Type" = WhseActivLine."Action Type"::Place) and (WhseActivLine."Bin Code" <> locate."Receipt Bin Code") and (whseActivLine."Zone Code" = locate.CrossDockZone) then begin
                ...
                //else
                //  UpdateBack2BackOrder(WhseActivLine); //Ravi for Auto Pick update
    ...
    begin
        with WhseActivLine do begin
            if "Original Breakbulk" then exit;
            ...
            case WhseDocType2 of
                "Whse. Document Type"::Shipment:
                    if ("Action Type" <> "Action Type"::Take) and ("Breakbulk No." = 0) then begin
                        UpdateWhseShptLine("Whse. Document No.", "Whse. Document Line No.", "Qty. to Handle", "Qty. to Handle (Base)", "Qty. per Unit of Measure");
                        RegisterWhseItemTrkgLine(WhseActivLine);
                    end;
                "Whse. Document Type"::"Internal Pick":
                    if ("Action Type" <> "Action Type"::Take) and ("Breakbulk No." = 0) then begin
                        UpdateWhseIntPickLine("Whse. Document No.", "Whse. Document Line No.", "Qty. to Handle", "Qty. to Handle (Base)", "Qty. per Unit of Measure");
                        RegisterWhseItemTrkgLine(WhseActivLine);
                    end;
                "Whse. Document Type"::Production:
                    if ("Action Type" <> "Action Type"::Take) and ("Breakbulk No." = 0) then begin
                        UpdateProdCompLine("Source Subtype", "Source No.", "Source Line No.", "Source Subline No.", "Qty. to Handle", "Qty. to Handle (Base)", "Qty. per Unit of Measure");
                        RegisterWhseItemTrkgLine(WhseActivLine);
                    end;
                "Whse. Document Type"::Assembly:
                    if ("Action Type" <> "Action Type"::Take) and ("Breakbulk No." = 0) then begin
                        UpdateAssemblyLine("Source Subtype", "Source No.", "Source Line No.", "Qty. to Handle", "Qty. to Handle (Base)", "Qty. per Unit of Measure");
                        RegisterWhseItemTrkgLine(WhseActivLine);
                    end;
                "Whse. Document Type"::Receipt:
                    if "Action Type" <> "Action Type"::Place then begin
                        UpdatePostedWhseRcptLine("Whse. Document No.", "Whse. Document Line No.", "Qty. to Handle", "Qty. to Handle (Base)", "Qty. per Unit of Measure");
                        RegisterWhseItemTrkgLine(WhseActivLine);
                    end;
                "Whse. Document Type"::"Internal Put-away":
                    if "Action Type" <> "Action Type"::Take then begin
                        UpdateWhseIntPutAwayLine("Whse. Document No.", "Whse. Document Line No.", "Qty. to Handle", "Qty. to Handle (Base)", "Qty. per Unit of Measure");
                        RegisterWhseItemTrkgLine(WhseActivLine);
                    end;
            end;
        end;
        if WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Invt. Movement" then UpdateSourceDocForInvtMovement(WhseActivLine);
    end;
    ...
    begin
        with WhseActivLine do case "Whse. Document Type" of
                                  "Whse. Document Type"::Shipment:
                                      if "Action Type" <> "Action Type"::Take then begin
                                  ...
                                  "Whse. Document Type"::Receipt:
                                      if "Action Type" <> "Action Type"::Place then begin
                                          ...
                                          if PostedWhseRcptHeader."Document Status" = PostedWhseRcptHeader."Document Status"::"Completely Put Away" then begin
                                              ...
                                              ItemTrackingMgt.DeleteWhseItemTrkgLines(DATABASE::"Posted Whse. Receipt Line", 0, PostedWhseRcptHeader."No.", '', 0, 0, '', false);
                                          end;
                                      end;
                                  "Whse. Document Type"::"Internal Pick":
                                      if "Action Type" <> "Action Type"::Take then begin
                                          ...
                                          if WhseInternalPickLine.FindFirst then begin
                                              ...
                                              if WhseInternalPickHeader."Document Status" = WhseInternalPickHeader."Document Status"::"Completely Picked" then begin
                                  ...
                                  "Whse. Document Type"::"Internal Put-away":
                                      if "Action Type" <> "Action Type"::Take then begin
                                          ...
                                          if WhseInternalPutAwayLine.FindFirst then begin
                                              ...
                                              if WhseInternalPutAwayHeader."Document Status" = WhseInternalPutAwayHeader."Document Status"::"Completely Put Away" then begin
                                  ...
                                  "Whse. Document Type"::Production:
                                      if "Action Type" <> "Action Type"::Take then begin
                                          ...
                                          if ProdOrder."Completely Picked" then begin
                                              ...
                                              ItemTrackingMgt.DeleteWhseItemTrkgLines(DATABASE::"Prod. Order Component", "Source Subtype", "Source No.", '', 0, 0, '', false);
                                          end;
                                      end;
                                  "Whse. Document Type"::Assembly:
                                      if "Action Type" <> "Action Type"::Take then begin
                                          ...
                                          if AssemblyHeader.CompletelyPicked then begin
                                              ...
                                              WhsePickRqst.ModifyAll("Completely Picked", true);
                                              ItemTrackingMgt.DeleteWhseItemTrkgLines(DATABASE::"Assembly Line", "Source Subtype", "Source No.", '', 0, 0, '', false);
                                          end;
                                      end;
            end;
    end;

    procedure UpdateWhseShptLine(WhseDocNo: Code[20];
    ...
    begin
        ...
        WhseShptLine."Qty. Picked (Base)" := WhseShptLine."Qty. Picked (Base)" + QtyToHandleBase;
        if QtyPerUOM = WhseShptLine."Qty. per Unit of Measure" then
            WhseShptLine."Qty. Picked" := WhseShptLine."Qty. Picked" + QtyToHandle
        else
            WhseShptLine."Qty. Picked" := Round(WhseShptLine."Qty. Picked" + QtyToHandleBase / QtyPerUOM);
        WhseShptLine."Completely Picked" := (WhseShptLine."Qty. Picked" = WhseShptLine.Quantity) or (WhseShptLine."Qty. Picked (Base)" = WhseShptLine."Qty. (Base)");
        // Handle rounding residual when completely picked
        if WhseShptLine."Completely Picked" and (WhseShptLine."Qty. Picked" <> WhseShptLine.Quantity) then WhseShptLine."Qty. Picked" := WhseShptLine.Quantity;
    ...
    local procedure UpdatePostedWhseRcptLine(WhseDocNo: Code[20];
    WhseDocLineNo: Integer;
    QtyToHandle: Decimal;
    QtyToHandleBase: Decimal;
    QtyPerUOM: Decimal)
    begin
        ...
        PostedWhseRcptLine."Qty. Put Away (Base)" := PostedWhseRcptLine."Qty. Put Away (Base)" + QtyToHandleBase;
        if QtyPerUOM = PostedWhseRcptLine."Qty. per Unit of Measure" then
            PostedWhseRcptLine."Qty. Put Away" := PostedWhseRcptLine."Qty. Put Away" + QtyToHandle
        else
            PostedWhseRcptLine."Qty. Put Away" := Round(PostedWhseRcptLine."Qty. Put Away" + QtyToHandleBase / PostedWhseRcptLine."Qty. per Unit of Measure");
        PostedWhseRcptLine.Status := PostedWhseRcptLine.GetLineStatus;
        PostedWhseRcptLine.Modify;
    end;

    local procedure UpdateWhseIntPickLine(WhseDocNo: Code[20];
    WhseDocLineNo: Integer;
    QtyToHandle: Decimal;
    QtyToHandleBase: Decimal;
    QtyPerUOM: Decimal)
    begin
        ...
        if WhseInternalPickLine."Qty. (Base)" = WhseInternalPickLine."Qty. Picked (Base)" + QtyToHandleBase then
            WhseInternalPickLine.Delete
        else begin
            WhseInternalPickLine."Qty. Picked (Base)" := WhseInternalPickLine."Qty. Picked (Base)" + QtyToHandleBase;
            if QtyPerUOM = WhseInternalPickLine."Qty. per Unit of Measure" then
                WhseInternalPickLine."Qty. Picked" := WhseInternalPickLine."Qty. Picked" + QtyToHandle
            else
                WhseInternalPickLine."Qty. Picked" := Round(WhseInternalPickLine."Qty. Picked" + QtyToHandleBase / QtyPerUOM);
            WhseInternalPickLine.Validate("Qty. Outstanding", WhseInternalPickLine."Qty. Outstanding" - QtyToHandle);
            WhseInternalPickLine.Status := WhseInternalPickLine.CalcStatusPickLine;
            WhseInternalPickLine.Modify;
        end;
    end;

    local procedure UpdateWhseIntPutAwayLine(WhseDocNo: Code[20];
    WhseDocLineNo: Integer;
    QtyToHandle: Decimal;
    QtyToHandleBase: Decimal;
    QtyPerUOM: Decimal)
    begin
        ...
        if WhseInternalPutAwayLine."Qty. (Base)" = WhseInternalPutAwayLine."Qty. Put Away (Base)" + QtyToHandleBase then
            WhseInternalPutAwayLine.Delete
        else begin
            WhseInternalPutAwayLine."Qty. Put Away (Base)" := WhseInternalPutAwayLine."Qty. Put Away (Base)" + QtyToHandleBase;
            if QtyPerUOM = WhseInternalPutAwayLine."Qty. per Unit of Measure" then
                WhseInternalPutAwayLine."Qty. Put Away" := WhseInternalPutAwayLine."Qty. Put Away" + QtyToHandle
            else
                WhseInternalPutAwayLine."Qty. Put Away" := Round(WhseInternalPutAwayLine."Qty. Put Away" + QtyToHandleBase / WhseInternalPutAwayLine."Qty. per Unit of Measure");
            WhseInternalPutAwayLine.Validate("Qty. Outstanding", WhseInternalPutAwayLine."Qty. Outstanding" - QtyToHandle);
            WhseInternalPutAwayLine.Status := WhseInternalPutAwayLine.CalcStatusPutAwayLine;
            WhseInternalPutAwayLine.Modify;
        end;
    end;

    local procedure UpdateProdCompLine(SourceSubType: Option;
    SourceNo: Code[20];
    ...
    begin
        ...
        ProdCompLine."Qty. Picked (Base)" := ProdCompLine."Qty. Picked (Base)" + QtyToHandleBase;
        if QtyPerUOM = ProdCompLine."Qty. per Unit of Measure" then
            ProdCompLine."Qty. Picked" := ProdCompLine."Qty. Picked" + QtyToHandle
        else
            ProdCompLine."Qty. Picked" := Round(ProdCompLine."Qty. Picked" + QtyToHandleBase / QtyPerUOM);
        ProdCompLine."Completely Picked" := ProdCompLine."Qty. Picked" = ProdCompLine."Expected Quantity";
        ProdCompLine.Modify;
    end;

    local procedure UpdateAssemblyLine(SourceSubType: Option;
    SourceNo: Code[20];
    SourceLineNo: Integer;
    QtyToHandle: Decimal;
    QtyToHandleBase: Decimal;
    ...
    begin
        ...
        AssemblyLine."Qty. Picked (Base)" := AssemblyLine."Qty. Picked (Base)" + QtyToHandleBase;
        if QtyPerUOM = AssemblyLine."Qty. per Unit of Measure" then
            AssemblyLine."Qty. Picked" := AssemblyLine."Qty. Picked" + QtyToHandle
        else
            AssemblyLine."Qty. Picked" := Round(AssemblyLine."Qty. Picked" + QtyToHandleBase / QtyPerUOM);
        AssemblyLine.Modify;
    end;
    ...
    procedure GetItemUnitOfMeasure(ItemNo: Code[20];
    UOMCode: Code[10])
    begin
        if (ItemUnitOfMeasure."Item No." <> ItemNo) or (ItemUnitOfMeasure.Code <> UOMCode) then if not ItemUnitOfMeasure.Get(ItemNo, UOMCode) then ItemUnitOfMeasure.Init;
    end;
    ...
    begin
        with WhseActivLine do begin
            ...
            if not TempBinContentBuffer.Get("Location Code", "Bin Code", "Item No.", "Variant Code", UOMCode, "Lot No.", "Serial No.") then begin
            ...
            Sign := 1;
            if "Action Type" = "Action Type"::Take then Sign := -1;
            ...
            TempBinContentBuffer."Qty. Outstanding (Base)" := TempBinContentBuffer."Qty. Outstanding (Base)" + Sign * "Qty. Outstanding (Base)";
        ...
        with TempBinContentBuffer do begin
            ...
            if Find('-') then
                repeat
                    ...
                    Bin.CheckIncreaseBin("Bin Code", '', "Qty. to Handle (Base)", Cubage, Weight, Cubage, Weight, true, false);
                    SetFilter("Qty. to Handle (Base)", '>0');
                    Find('+');
                    SetRange("Bin Code");
                until Next = 0;
            ...
            if Find('-') then
                repeat
                    if "Qty. to Handle (Base)" < 0 then begin
                        ...
                        if WhseLNRequired then BinContent.SetRange("Lot No. Filter", "Lot No.");
                        if WhseSNRequired then BinContent.SetRange("Serial No. Filter", "Serial No.");
    ...
    begin
        with TempBinContentBuffer do begin
            BreakBulkWhseActivLine.SetCurrentKey("Item No.", "Bin Code", "Location Code", "Action Type", "Variant Code", "Unit of Measure Code", "Breakbulk No.", "Activity Type", "Lot No.", "Serial No.");
    ...
    var
        ...
        WhseLNRequired: Boolean;
    begin
        if not ((WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::Pick) or (WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Invt. Movement")) then exit;
        if WhseActivLine.Find('-') then
            repeat
                TempWhseActivLine := WhseActivLine;
                if not (TempWhseActivLine."Action Type" = TempWhseActivLine."Action Type"::Place) then TempWhseActivLine.Insert;
            until WhseActivLine.Next = 0;
            ...
            repeat
                ...
                if WhseSNRequired or WhseLNRequired then
                    repeat
                        ...
                        if WhseLNRequired then TempWhseActivLine.TestField("Lot No.");
        ...
        if TempWhseActivLine.Find('-') then
            repeat
                ItemTrackingMgt.CheckWhseItemTrkgSetup(TempWhseActivLine."Item No.", WhseSNRequired, WhseLNRequired, false);
                // Per document
                TempWhseActivLine.SetSourceFilter(TempWhseActivLine."Source Type", TempWhseActivLine."Source Subtype", TempWhseActivLine."Source No.", TempWhseActivLine."Source Line No.", TempWhseActivLine."Source Subline No.", false);
                repeat // Per Lot/SN
                    TempWhseActivLine.SetRange("Item No.", TempWhseActivLine."Item No.");
                    ...
                    if QtyToRegisterBase > QtyAvailToRegisterBase then QtyAvailToInsertBase -= QtyToRegisterBase - QtyAvailToRegisterBase;
                    OnBeforeCheckQtyAvailToInsertBase(TempWhseActivLine, QtyAvailToInsertBase);
                    if QtyAvailToInsertBase < 0 then Error(Text004);
                    if (TempWhseActivLine."Serial No." <> '') or (TempWhseActivLine."Lot No." <> '') then if not IsQtyAvailToPickNonSpecificReservation(TempWhseActivLine, WhseSNRequired, WhseLNRequired, QtyToRegisterBase) then AvailabilityError(TempWhseActivLine);
    ...
    begin
        ...
        if not (WhseSNRequired or WhseLNRequired) then exit;
        ...
        if QtyToRegisterBase > 0 then begin
            if (WhseActivLine2."Activity Type" = WhseActivLine2."Activity Type"::Pick) or (WhseActivLine2."Activity Type" = WhseActivLine2."Activity Type"::"Invt. Movement") then InsertRegWhseItemTrkgLine(WhseActivLine2, QtyToRegisterBase);
            if (WhseActivLine2."Whse. Document Type" in [WhseActivLine2."Whse. Document Type"::Shipment, WhseActivLine2."Whse. Document Type"::Production, WhseActivLine2."Whse. Document Type"::Assembly]) or ((WhseActivLine2."Activity Type" = WhseActivLine2."Activity Type"::"Invt. Movement") and (WhseActivLine2."Source Type" > 0)) then begin
                if (WhseActivLine2."Whse. Document Type" = WhseActivLine2."Whse. Document Type"::Shipment) and WhseActivLine2."Assemble to Order" then
                ...
                case WhseDocType2 of
                    ...
                    WhseActivLine2."Whse. Document Type"::Production:
                        begin
                            ProdOrderComp.Get(WhseActivLine2."Source Subtype", WhseActivLine2."Source No.", WhseActivLine2."Source Line No.", WhseActivLine2."Source Subline No.");
                            DueDate := ProdOrderComp."Due Date";
                        end;
                    WhseActivLine2."Whse. Document Type"::Assembly:
                        begin
                            AssemblyLine.Get(WhseActivLine2."Source Subtype", WhseActivLine2."Source No.", WhseActivLine2."Source Line No.");
                ...
                TempTrackingSpecification.SetTracking(WhseActivLine2."Serial No.", WhseActivLine2."Lot No.", WhseActivLine2."Warranty Date", WhseActivLine2."Expiration Date");
    ...
    local procedure InitTempTrackingSpecification(WhseActivLine2: Record "Warehouse Activity Line";
    var TempTrackingSpecification: Record "Tracking Specification" temporary) QtyToRegisterBase: Decimal
    var
    ...
    begin
        ...
        case WhseDocType2 of
            WhseActivLine."Whse. Document Type"::Receipt:
                if WhsePostedRcptLine.Get(WhseActivLine."Whse. Document No.", WhseActivLine."Whse. Document Line No.") then
                    exit(WhsePostedRcptLine."Qty. (Base)");
            WhseActivLine."Whse. Document Type"::Shipment:
                if WhseShipmentLine.Get(WhseActivLine."Whse. Document No.", WhseActivLine."Whse. Document Line No.") then
                    exit(WhseShipmentLine."Qty. (Base)");
            WhseActivLine."Whse. Document Type"::"Internal Put-away":
                if WhseIntPutAwayLine.Get(WhseActivLine."Whse. Document No.", WhseActivLine."Whse. Document Line No.") then
                    exit(WhseIntPutAwayLine."Qty. (Base)");
            WhseActivLine."Whse. Document Type"::"Internal Pick":
                if WhseIntPickLine.Get(WhseActivLine."Whse. Document No.", WhseActivLine."Whse. Document Line No.") then
                    exit(WhseIntPickLine."Qty. (Base)");
            WhseActivLine."Whse. Document Type"::Production:
                if ProdOrderComponent.Get(WhseActivLine."Source Subtype", WhseActivLine."Source No.", WhseActivLine."Source Line No.", WhseActivLine."Source Subline No.") then
                    exit(ProdOrderComponent."Expected Qty. (Base)");
            WhseActivLine."Whse. Document Type"::Assembly:
                if AssemblyLine.Get(WhseActivLine."Source Subtype", WhseActivLine."Source No.", WhseActivLine."Source Line No.") then
                    exit(AssemblyLine."Quantity (Base)");
            WhseActivLine."Whse. Document Type"::"Movement Worksheet":
                if WhseMovementWksh.Get(WhseActivLine."Whse. Document No.", WhseActivLine."Source No.", WhseActivLine."Location Code", WhseActivLine."Source Line No.") then
    ...
    local procedure CalcQtyReservedOnInventory(WhseActivLine: Record "Warehouse Activity Line";
    SNRequired: Boolean;
    LNRequired: Boolean)
    begin
        with WhseActivLine do begin
    ...
    local procedure InsertRegWhseItemTrkgLine(WhseActivLine: Record "Warehouse Activity Line";
    QtyToRegisterBase: Decimal)
    var
    ...
    begin
        with WhseItemTrkgLine2 do begin
            ...
            SetTracking(WhseActivLine."Serial No.", WhseActivLine."Lot No.", WhseActivLine."Warranty Date", WhseActivLine."Expiration Date");
    ...
    procedure SetPointer(WhseActivLine: Record "Warehouse Activity Line";
    var WhseItemTrkgLine: Record "Whse. Item Tracking Line")
    var
        WhseDocType2: Option;
    begin
        with WhseActivLine do begin
            ...
            case WhseDocType2 of
                "Whse. Document Type"::Receipt:
                    WhseItemTrkgLine.SetSource(DATABASE::"Posted Whse. Receipt Line", 0, "Whse. Document No.", "Whse. Document Line No.", '', 0);
                "Whse. Document Type"::Shipment:
                    WhseItemTrkgLine.SetSource(DATABASE::"Warehouse Shipment Line", 0, "Whse. Document No.", "Whse. Document Line No.", '', 0);
                "Whse. Document Type"::"Internal Put-away":
                    WhseItemTrkgLine.SetSource(DATABASE::"Whse. Internal Put-away Line", 0, "Whse. Document No.", "Whse. Document Line No.", '', 0);
                "Whse. Document Type"::"Internal Pick":
                    WhseItemTrkgLine.SetSource(DATABASE::"Whse. Internal Pick Line", 0, "Whse. Document No.", "Whse. Document Line No.", '', 0);
                "Whse. Document Type"::Production:
                    WhseItemTrkgLine.SetSource(DATABASE::"Prod. Order Component", "Source Subtype", "Source No.", "Source Subline No.", '', "Source Line No.");
                "Whse. Document Type"::Assembly:
                    WhseItemTrkgLine.SetSource(DATABASE::"Assembly Line", "Source Subtype", "Source No.", "Source Line No.", '', 0);
                "Whse. Document Type"::"Movement Worksheet":
                    WhseItemTrkgLine.SetSource(DATABASE::"Whse. Worksheet Line", 0, "Source No.", "Whse. Document Line No.", CopyStr("Whse. Document No.", 1, MaxStrLen(WhseItemTrkgLine."Source Batch Name")), 0);
            end;
    ...
    procedure SetPointerFilter(WhseActivLine: Record "Warehouse Activity Line";
    var WhseItemTrkgLine: Record "Whse. Item Tracking Line")
    var
        WhseItemTrkgLine2: Record "Whse. Item Tracking Line";
    begin
        SetPointer(WhseActivLine, WhseItemTrkgLine2);
        WhseItemTrkgLine.SetSourceFilter(WhseItemTrkgLine2."Source Type", WhseItemTrkgLine2."Source Subtype", WhseItemTrkgLine2."Source ID", WhseItemTrkgLine2."Source Ref. No.", true);
    ...
    local procedure CalcTotalAvailQtyToPick(WhseActivLine: Record "Warehouse Activity Line";
    SNRequired: Boolean;
    LNRequired: Boolean): Decimal
    var
    ...
    begin
        with WhseActivLine do begin
            ...
            LocationGet("Location Code");
            if Location."Directed Put-away and Pick" or ("Activity Type" = "Activity Type"::"Invt. Movement") then begin
                ...
                if BinTypeFilter <> '' then WhseEntry.SetFilter("Bin Type Code", '<>%1', BinTypeFilter); // Pick from all but Receive area
                WhseEntry.CalcSums("Qty. (Base)");
                QtyOnPickBinsBase := WhseEntry."Qty. (Base)";
                QtyOnOutboundBinsBase := CreatePick.CalcQtyOnOutboundBins("Location Code", "Item No.", "Variant Code", "Lot No.", "Serial No.", true);
                QtyOnDedicatedBinsBase := WhseAvailMgt.CalcQtyOnDedicatedBins("Location Code", "Item No.", "Variant Code", "Lot No.", "Serial No.");
                SubTotalBase := QtyInWhseBase - QtyOnPickBinsBase - QtyOnOutboundBinsBase - QtyOnDedicatedBinsBase;
                if "Activity Type" <> "Activity Type"::"Invt. Movement" then SubTotalBase -= Abs(Item."Reserved Qty. on Inventory");
                if SubTotalBase < 0 then begin
                    CreatePick.FilterWhsePickLinesWithUndefinedBin(WarehouseActivityLine, "Item No.", "Location Code", "Variant Code", LNRequired, "Lot No.", SNRequired, "Serial No.");
                    ...
                    QtyReservedOnPickShipBase := WhseAvailMgt.CalcReservQtyOnPicksShips("Location Code", "Item No.", "Variant Code", TempWhseActivLine2);
                    LineReservedQtyBase := WhseAvailMgt.CalcLineReservedQtyOnInvt("Source Type", "Source Subtype", "Source No.", "Source Line No.", "Source Subline No.", true, '', '', TempWhseActivLine2);
                    if Abs(SubTotalBase) < QtyReservedOnPickShipBase + LineReservedQtyBase then QtyReservedOnPickShipBase := Abs(SubTotalBase) - LineReservedQtyBase;
                    TotalAvailQtyBase := QtyOnPickBinsBase + SubTotalBase + QtyReservedOnPickShipBase + LineReservedQtyBase;
                end
                else
                    TotalAvailQtyBase := QtyOnPickBinsBase;
            end
            else begin
                ItemLedgEntry.SetCurrentKey("Item No.", Open, "Variant Code", Positive, "Location Code", "Posting Date", "Expiration Date", "Lot No.", "Serial No.");
                ...
                LineReservedQtyBase := WhseAvailMgt.CalcLineReservedQtyOnInvt("Source Type", "Source Subtype", "Source No.", "Source Line No.", "Source Subline No.", false, '', '', TempWhseActivLine2);
                TotalAvailQtyBase := QtyInWhseBase - QtyPickedNotShipped - Abs(Item."Reserved Qty. on Inventory") + LineReservedQtyBase;
            end;
            exit(TotalAvailQtyBase);
        end;
    end;

    local procedure IsQtyAvailToPickNonSpecificReservation(WhseActivLine: Record "Warehouse Activity Line";
    SNRequired: Boolean;
    LNRequired: Boolean;
    QtyToRegister: Decimal): Boolean
    var
        QtyAvailToPick: Decimal;
    begin
        QtyAvailToPick := CalcTotalAvailQtyToPick(WhseActivLine, SNRequired, LNRequired);
        if QtyAvailToPick < QtyToRegister then if ReleaseNonSpecificReservations(WhseActivLine, SNRequired, LNRequired, QtyToRegister - QtyAvailToPick) then QtyAvailToPick := CalcTotalAvailQtyToPick(WhseActivLine, SNRequired, LNRequired);
        exit(QtyAvailToPick >= QtyToRegister);
    end;

    local procedure CalcQtyPickedNotShipped(WhseActivLine: Record "Warehouse Activity Line";
    SNRequired: Boolean;
    LNRequired: Boolean) QtyBasePicked: Decimal
    var
    ...
    begin
        with WhseActivLine do begin
            ...
            if ReservEntry.Find('-') then
                repeat
                    if not ((ReservEntry."Source Type" = "Source Type") and (ReservEntry."Source Subtype" = "Source Subtype") and (ReservEntry."Source ID" = "Source No.") and ((ReservEntry."Source Ref. No." = "Source Line No.") or (ReservEntry."Source Ref. No." = "Source Subline No."))) and not ReservEntry.Positive then QtyBasePicked := QtyBasePicked + Abs(ReservEntry."Quantity (Base)");
    ...
    local procedure UpdateTempTracking(WhseActivLine2: Record "Warehouse Activity Line";
    QtyToHandleBase: Decimal;
    var TempTrackingSpecification: Record "Tracking Specification" temporary): Boolean
    var
        NextEntryNo: Integer;
        Inserted: Boolean;
    begin
    ...
    local procedure CheckItemTrackingInfoBlocked(ItemNo: Code[20];
    VariantCode: Code[10];
    SerialNo: Code[50];
    LotNo: Code[50])
    var
    ...
    begin
        if (SerialNo = '') and (LotNo = '') then exit;
        if SerialNo <> '' then if SerialNoInfo.Get(ItemNo, VariantCode, SerialNo) then SerialNoInfo.TestField(Blocked, false);
        if LotNo <> '' then if LotNoInfo.Get(ItemNo, VariantCode, LotNo) then LotNoInfo.TestField(Blocked, false);
    end;

    local procedure UpdateWindow(ControlNo: Integer;
    Value: Code[20])
    begin
        ...
        with WhseActivHeader do begin
            ...
            if WhseActivLine.Find('-') then
                repeat
                    ...
                    if (WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::Pick) and (WhseActivLine."Destination Type" = WhseActivLine."Destination Type"::Customer) then begin
                    ...
                    if ((WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::Pick) or (WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Invt. Pick") or (WhseActivLine."Activity Type" = WhseActivLine."Activity Type"::"Invt. Movement")) and (WhseActivLine."Action Type" = WhseActivLine."Action Type"::Take) then CheckItemTrackingInfoBlocked(WhseActivLine."Item No.", WhseActivLine."Variant Code", WhseActivLine."Serial No.", WhseActivLine."Lot No.");
                until WhseActivLine.Next = 0;
    ...
    begin
        if (WhseActivityLine."Action Type" = WhseActivityLine."Action Type"::Take) or (WhseActivityLine."Source Document" = WhseActivityLine."Source Document"::" ") then exit;
        with WhseActivityLine do case "Source Document" of
    ...
    begin
        TempTrackingSpecification.Reset;
        if TempTrackingSpecification.FindLast then exit(TempTrackingSpecification."Entry No." + 1);
        exit(1);
    end;
    ...
    begin
        if TempWhseActivLineToReserve.FindSet then
            repeat
                SalesLine.Get(SalesLine."Document Type"::Order, TempWhseActivLineToReserve."Source No.", TempWhseActivLineToReserve."Source Line No.");
                if not IsSalesLineCompletelyReserved(SalesLine, TempWhseActivLineToReserve."Serial No.", TempWhseActivLineToReserve."Lot No.") then begin
                    ReservMgt.SetSalesLine(SalesLine);
                    ReservMgt.SetSerialLotNo(TempWhseActivLineToReserve."Serial No.", TempWhseActivLineToReserve."Lot No.");
                    ReservMgt.AutoReserve(FullAutoReservation, '', SalesLine."Shipment Date", TempWhseActivLineToReserve."Qty. to Handle", TempWhseActivLineToReserve."Qty. to Handle (Base)");
                end;
    ...
    begin
        if TempWhseActivLineToReserve.FindSet then
            repeat
                SalesLine.Get(SalesLine."Document Type"::Order, TempWhseActivLineToReserve."Source No.", TempWhseActivLineToReserve."Source Line No.");
                ReservationEntry.SetSourceFilter(DATABASE::"Sales Line", SalesLine."Document Type", SalesLine."Document No.", SalesLine."Line No.", true);
                ...
                if ReservationEntry.FindSet then begin
                    if not IsConfirmed and GuiAllowed then if not Confirm(OrderToOrderBindingOnSalesLineQst) then Error(RegisterInterruptedErr);
                    IsConfirmed := true;
                    repeat
    ...
    local procedure CopyWhseActivityLineToReservBuf(var TempWhseActivLineToReserve: Record "Warehouse Activity Line" temporary;
    WhseActivLine: Record "Warehouse Activity Line")
    begin
        if not IsPickPlaceForSalesOrderTrackedItem(WhseActivLine) then exit;
        TempWhseActivLineToReserve.TransferFields(WhseActivLine);
        TempWhseActivLineToReserve.Insert;
    end;

    procedure ReleaseNonSpecificReservations(WhseActivLine: Record "Warehouse Activity Line";
    SNRequired: Boolean;
    LNRequired: Boolean;
    QtyToRelease: Decimal): Boolean
    var
        LateBindingMgt1: Codeunit "Late Binding ManagementN";
        xReservedQty: Decimal;
    begin
        if QtyToRelease <= 0 then exit;
        CalcQtyReservedOnInventory(WhseActivLine, SNRequired, LNRequired);
        if LNRequired or SNRequired then
            if Item."Reserved Qty. on Inventory" > 0 then begin
                ...
                LateBindingMgt1.ReleaseForReservation(WhseActivLine."Item No.", WhseActivLine."Variant Code", WhseActivLine."Location Code", WhseActivLine."Serial No.", WhseActivLine."Lot No.", QtyToRelease);
                Item.CalcFields("Reserved Qty. on Inventory");
            end;
    ...
    begin
        if WhseActivLine."Serial No." <> '' then Error(Text005, WhseActivLine.FieldCaption("Serial No."), WhseActivLine."Serial No.");
        if WhseActivLine."Lot No." <> '' then Error(Text005, WhseActivLine.FieldCaption("Lot No."), WhseActivLine."Lot No.");
    end;

    local procedure IsPickPlaceForSalesOrderTrackedItem(WhseActivityLine: Record "Warehouse Activity Line"): Boolean
    begin
        exit((WhseActivityLine."Activity Type" = WhseActivityLine."Activity Type"::Pick) and (WhseActivityLine."Action Type" in [WhseActivityLine."Action Type"::Place, WhseActivityLine."Action Type"::" "]) and (WhseActivityLine."Source Document" = WhseActivityLine."Source Document"::"Sales Order") and ((WhseActivityLine."Serial No." <> '') or (WhseActivityLine."Lot No." <> '')));
    end;

    local procedure IsSalesLineCompletelyReserved(SalesLine: Record "Sales Line";
    SerialNo: Code[50];
    LotNo: Code[50]): Boolean
    var
        ReservationEntry: Record "Reservation Entry";
    begin
        ...
        if ReservationEntry.IsEmpty then exit(false);
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterCreateRegActivLine(var WarehouseActivityLine: Record "Warehouse Activity Line";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckLines(var WarehouseActivityHeader: Record "Warehouse Activity Header";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterRegWhseItemTrkgLine(var WhseActivLine2: Record "Warehouse Activity Line";
    var TempTrackingSpecification: Record "Tracking Specification" temporary)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInsRegWhseItemTrkgLine(var WhseActivLine2: Record "Warehouse Activity Line";
    var WhseItemTrkgLine: Record "Whse. Item Tracking Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterRegisteredInvtMovementHdrInsert(var RegisteredInvtMovementHdr: Record "Registered Invt. Movement Hdr.";
    WarehouseActivityHeader: Record "Warehouse Activity Header")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterRegisteredInvtMovementLineInsert(var RegisteredInvtMovementLine: Record "Registered Invt. Movement Line";
    WarehouseActivityLine: Record "Warehouse Activity Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterRegisteredWhseActivHeaderInsert(var RegisteredWhseActivityHdr: Record "Registered Whse. Activity Hdr.";
    WarehouseActivityHeader: Record "Warehouse Activity Header")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterRegisteredWhseActivLineInsert(var RegisteredWhseActivityLine: Record "Registered Whse. Activity Line";
    WarehouseActivityLine: Record "Warehouse Activity Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRegWhseItemTrkgLine(var WhseActivLine2: Record "Warehouse Activity Line";
    var TempTrackingSpecification: Record "Tracking Specification" temporary)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRegisteredInvtMovementHdrInsert(var RegisteredInvtMovementHdr: Record "Registered Invt. Movement Hdr.";
    WarehouseActivityHeader: Record "Warehouse Activity Header")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRegisteredInvtMovementLineInsert(var RegisteredInvtMovementLine: Record "Registered Invt. Movement Line";
    WarehouseActivityLine: Record "Warehouse Activity Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRegisteredWhseActivHeaderInsert(var RegisteredWhseActivityHdr: Record "Registered Whse. Activity Hdr.";
    WarehouseActivityHeader: Record "Warehouse Activity Header")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeRegisteredWhseActivLineInsert(var RegisteredWhseActivityLine: Record "Registered Whse. Activity Line";
    WarehouseActivityLine: Record "Warehouse Activity Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckQtyAvailToInsertBase(var TempWhseActivLine: Record "Warehouse Activity Line" temporary;
    var QtyAvailToInsertBase: Decimal)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeUpdateWhseShptLine(var WarehouseShipmentLine: Record "Warehouse Shipment Line";
    QtyToHandle: Decimal;
    QtyToHandleBase: Decimal;
    QtyPerUOM: Decimal)
    begin
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeWhseActivHeaderDelete(var WarehouseActivityHeader: Record "Warehouse Activity Header";
    var SkipDelete: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInitRegActLine(var WarehouseActivityLine: Record "Warehouse Activity Line";
    var RegisteredWhseActivityLine: Record "Registered Whse. Activity Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInitRegInvtMovementLine(var WarehouseActivityLine: Record "Warehouse Activity Line";
    var RegisteredInvtMovementLine: Record "Registered Invt. Movement Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCreateRegActivHeader(var WhseActivityHeader: Record "Warehouse Activity Header";
    var IsHandled: Boolean;
    var RegisteredWhseActivityHdr: Record "Registered Whse. Activity Hdr.";
    var RegisteredInvtMovementHdr: Record "Registered Invt. Movement Hdr.")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeWhseJnlRegisterLine(var WarehouseJournalLine: Record "Warehouse Journal Line";
    WarehouseActivityLine: Record "Warehouse Activity Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeTempTrackingSpecificationInsert(var TempTrackingSpecification: Record "Tracking Specification" temporary;
    WarehouseActivityLine: Record "Warehouse Activity Line")
    begin
    end;

    local procedure UpdateBack2BackOrder(var whsActPutawayLines: Record "Warehouse Activity Line")
    var
        whsPickLines: Record "Warehouse Activity Line";
        OrderTracking: Record "Order Tracking Table";
        WhsShipLine: Record "Warehouse Shipment Line";
        whsRecpLine: Record "Warehouse Receipt Line";
        whsPutLines1: Record "Warehouse Activity Line";
        binTbl: Record Bin;
    begin
        if (whsActPutawayLines."Source Type" = 39) and (whsActPutawayLines."Activity Type" = whsActPutawayLines."Activity Type"::"Put-away") and (whsActPutawayLines.GSOrderItemUID = '') and (whsActPutawayLines."Action Type" = whsActPutawayLines."Action Type"::Place) then BEGIN
            whsPickLines.Reset;
            whsPickLines.SetCurrentKey(GSOrderItemUID);
            whsPickLines.SetRange("Item No.", whsActPutawayLines."Item No.");
            whsPickLines.SETRANGE("Activity Type", whsPickLines."Activity Type"::Pick);
            whsPickLines.SetRange("Action Type", whsPickLines."Action Type"::Take);
            whsPickLines.SetRange("Source Type", 37);
            whsPickLines.SetRange("Bin Code", '');
            whsPickLines.SetFilter(OrderStatus, '<>%1', whsPickLines.OrderStatus::Canceled);
            whsPickLines.SetRange("Location Code", whsActPutawayLines."Location Code");
            whsPickLines.SETRANGE("Due Date", CalcDate('-3D', Today), CalcDate('7D', Today));
            if whsPickLines.FindSet then
                repeat
                    binTbl.GET(whsPickLines."Location Code", whsActPutawayLines."Bin Code");
                    //if (whsPickLines."Action Type" = whsPickLines."Action Type"::Take) and (whsPickLines."Bin Code" = '') then
                    whsPickLines."Bin Code" := whsActPutawayLines."Bin Code";
                    whsPickLines."Zone Code" := binTbl."Zone Code";
                    whsPickLines.Validate("Qty. to Handle", 1);
                    whsPickLines.Modify;
                    IF OrderTracking.GET(whsPickLines.GSOrderItemUID) THEN BEGIN
                        OrderTracking."Pick No." := whsPickLines."No.";
                        OrderTracking."PO No." := whsActPutawayLines."Source No.";
                        OrderTracking.Modify;
                    END;
                until whsPickLines.Next = 0;
        END;
    end;


    local procedure UpdateCrossDockPickwrtOrdItem(var whsActPutawayLines: Record "Warehouse Activity Line")
    var
        whsPickLines: Record "Warehouse Activity Line";
        OrderTracking: Record "Order Tracking Table";
        WhsShipLine: Record "Warehouse Shipment Line";
    ...
    begin
        if (whsActPutawayLines."Source Type" = 39) and (whsActPutawayLines."Activity Type" = whsActPutawayLines."Activity Type"::"Put-away") and (whsActPutawayLines.GSOrderItemUID <> '') and (whsActPutawayLines."Action Type" = whsActPutawayLines."Action Type"::Place) then Begin
            whsPickLines.Reset;
            //whsPickLines.SetCurrentKey("Activity Type", "Location Code", GSOrderItemUID, "Action Type");
            whsPickLines.SetCurrentKey(GSOrderItemUID);
            whsPickLines.SetRange("Activity Type", whsPickLines."Activity Type"::Pick);
            whsPickLines.SetRange("Location Code", whsActPutawayLines."Location Code");
            whsPickLines.SetRange(GSOrderItemUID, whsActPutawayLines.GSOrderItemUID);
            whsPickLines.SetRange("Action Type", whsPickLines."Action Type"::Take);
            whsPickLines.SetFilter("Qty. Outstanding", '>0');
            if whsPickLines.FindFirst then Begin
                bins.get(whsPickLines."Location Code", whsActPutawayLines."Bin Code");
                whsPickLines."Bin Code" := whsActPutawayLines."Bin Code";
                whsPickLines."Zone Code" := bins."Zone Code";
                whsPickLines.Validate("Qty. to Handle", 1);
                whsPickLines.Modify;
                commit;
                IF OrderTracking.GET(whsPickLines.GSOrderItemUID) THEN //OrderTracking."Pick No." := whsPickLines."No.";
                                                                       //OrderTracking."PO No." := whsActPutawayLines."Source No.";
                    OrderTracking.ModifyAll("PO No.", whsActPutawayLines."Source No.");
            END;
        END;
        //until whsPickLines.Next = 0;
    end;
