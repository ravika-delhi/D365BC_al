codeunit 50051 "Get Source Doc. Outbound-N"
{
    trigger OnRun()
    begin
    end;
    ...
    begin
        if WarehouseRequest.IsEmpty then exit(false);
    ...
    begin
        ...
        if SourceDocSelection.RunModal <> ACTION::LookupOK then exit;
    ...
    begin
        ...
        if WhseRqst.IsEmpty then exit(false);
    ...
    procedure GetSingleWhsePickDoc(CurrentWhseWkshTemplate: Code[10];
    ...
    begin
        ...
        if WhsePickDocSelection.RunModal <> ACTION::LookupOK then exit;
        WhsePickDocSelection.GetResult(WhsePickRqst);
        GetWhseSourceDocuments.SetPickWkshName(CurrentWhseWkshTemplate, CurrentWhseWkshName, LocationCode);
    ...
    procedure CheckSalesHeader(SalesHeader: Record "Sales Header";
    ...
    begin
        ...
        with SalesHeader do begin
            if not ("Shipping Advice" = "Shipping Advice"::Complete) then exit(false);
            ...
            if SalesLine.FindSet then
                repeat
                    if SalesLine.IsInventoriableItem then SalesLine.Mark(true);
            ...
            if SalesLine.FindSet then begin
                ...
                repeat
                    ...
                    if SalesLine."Location Code" <> LocationCode then begin
                        if ShowError then Error(Text001, FieldCaption("Shipping Advice"), "Shipping Advice", SalesOrder.Caption, "No.", SalesLine.Type);
                        exit(true);
                    end;
                    if EqualItemVariant(CurrItemVariant, SalesLine."No.", SalesLine."Variant Code") then
                        ...
                        if CheckAvailability(CurrItemVariant, QtyOutstandingBase, SalesLine."Location Code", SalesOrder.Caption, DATABASE::"Sales Line", "Document Type", "No.", ShowError) then exit(true);
                        SetItemVariant(CurrItemVariant, SalesLine."No.", SalesLine."Variant Code");
                        QtyOutstandingBase := SalesLine."Outstanding Qty. (Base)";
                    end;
    ...
    procedure CheckTransferHeader(TransferHeader: Record "Transfer Header";
    ShowError: Boolean): Boolean
    var
            ...
            if TransferLine.FindSet then begin
                ...
                repeat
                    ...
                    if EqualItemVariant(CurrItemVariant, TransferLine."Item No.", TransferLine."Variant Code") then
                        ...
                        if CheckAvailability(CurrItemVariant, QtyOutstandingBase, TransferLine."Transfer-from Code", TransferOrder.Caption, DATABASE::"Transfer Line", 0, "No.", ShowError) then // outbound
                            exit(true);
                        SetItemVariant(CurrItemVariant, TransferLine."Item No.", TransferLine."Variant Code");
                        QtyOutstandingBase := TransferLine."Outstanding Qty. (Base)";
                    end;
    ...
    local procedure CheckAvailability(CurrItemVariant: Record "Item Variant";
    ...
    begin
        with Item do begin
            ...
            if Inventory - ("Reserved Qty. on Inventory" - QtyReservedForOrder) < QtyBaseNeeded then begin
                if ShowError then Error(Text002, CurrItemVariant."Item No.", LocationCode, FormCaption, SourceID);
                exit(true);
            end;
        end;
    end;
    ...
    begin
        if WhseRqst.FindSet then begin
            repeat
                if Location.RequireShipment(WhseRqst."Location Code") then LocationCode += WhseRqst."Location Code" + '|';
            until WhseRqst.Next = 0;
            if LocationCode <> '' then begin
                LocationCode := CopyStr(LocationCode, 1, StrLen(LocationCode) - 1);
                if LocationCode[1] = '|' then LocationCode := '''''' + LocationCode;
            end;
            WhseRqst.SetFilter("Location Code", LocationCode);
        end;
    end;

    local procedure SetItemVariant(var CurrItemVariant: Record "Item Variant";
    ItemNo: Code[20];
    VariantCode: Code[10])
    begin
    ...
    local procedure EqualItemVariant(CurrItemVariant: Record "Item Variant";
    ItemNo: Code[20];
    ...
    local procedure FindWarehouseRequestForSalesOrder(var WhseRqst: Record "Warehouse Request";
    SalesHeader: Record "Sales Header")
    begin
        with SalesHeader do begin
            TestField(Status, Status::Released);
            if WhseShpmntConflict("Document Type", "No.", "Shipping Advice") then Error(Text003, Format("Shipping Advice"));
    ...
    local procedure FindWarehouseRequestForPurchReturnOrder(var WhseRqst: Record "Warehouse Request";
    ...
    local procedure FindWarehouseRequestForOutbndTransferOrder(var WhseRqst: Record "Warehouse Request";
    ...
    local procedure FindWarehouseRequestForServiceOrder(var WhseRqst: Record "Warehouse Request";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckSalesHeader(var SalesHeader: Record "Sales Header";
    ShowError: Boolean)
    begin
    end;
}
