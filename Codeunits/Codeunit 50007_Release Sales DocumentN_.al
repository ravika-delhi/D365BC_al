codeunit 50007 "Release Sales DocumentN"
{
    ...
    begin
        with SalesHeader do begin
            ...
            if not (PreviewMode or SkipCheckReleaseRestrictions) then CheckSalesReleaseRestrictions;
            ...
            if not SalesLine.Find('-') then Error(Text001, "Document Type", "No.");
            InvtSetup.Get;
            if InvtSetup."Location Mandatory" then begin
                SalesLine.SetRange(Type, SalesLine.Type::Item);
                if SalesLine.FindSet then
                    repeat
                        if SalesLine.IsInventoriableItem then SalesLine.TestField("Location Code");
            ...
            if SalesSetup."Calc. Inv. Discount" then begin
                ...
                if PostingDate <> "Posting Date" then Validate("Posting Date", PostingDate);
            end;
            IsHandled := false;
            ...
            Modify(true);
            if NotOnlyDropShipment then if "Document Type" in ["Document Type"::Order, "Document Type"::"Return Order"] then WhseSalesRelease.Release(SalesHeader);
            OnAfterReleaseSalesDoc(SalesHeader, PreviewMode, LinesWereModified);
        end;
    end;
    ...
    begin
        ...
        with SalesHeader do begin
            ...
            Status := Status::Open;
            if "Document Type" <> "Document Type"::Order then ReopenATOs(SalesHeader);
            Modify(true);
            if "Document Type" in ["Document Type"::Order, "Document Type"::"Return Order"] then WhseSalesRelease.Reopen(SalesHeader);
        end;
    ...
    var
        PrepaymentMgt: Codeunit "Prepayment Mgt.";
    begin
        if PrepaymentMgt.TestSalesPrepayment(SalesHeader) then Error(UnpostedPrepaymentAmountsErr, SalesHeader."Document Type", SalesHeader."No.");
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        with SalesHeader do if ("Document Type" = "Document Type"::Order) and PrepaymentMgt.TestSalesPayment(SalesHeader) then begin
        ...
        if ApprovalsMgmt.IsSalesHeaderPendingApproval(SalesHeader) then Error(Text002);
        IsHandled := false;
        OnBeforePerformManualRelease(SalesHeader, PreviewMode, IsHandled);
        if IsHandled then exit;
        CODEUNIT.Run(CODEUNIT::"Release Sales DocumentN", SalesHeader);
    end;


    procedure PerformManualReopen(var SalesHeader: Record "Sales Header")
    begin
        if SalesHeader.Status = SalesHeader.Status::"Pending Approval" then Error(Text003);
    ...
    begin
        ...
        if SalesLine.FindSet then
            repeat
                if SalesLine.AsmToOrderExists(AsmHeader) then CODEUNIT.Run(CODEUNIT::"Release Assembly Document", AsmHeader);
            until SalesLine.Next = 0;
    end;
    ...
    begin
        ...
        if SalesLine.FindSet then
            repeat
                if SalesLine.AsmToOrderExists(AsmHeader) then ReleaseAssemblyDocument.Reopen(AsmHeader);
            until SalesLine.Next = 0;
    end;


    procedure ReleaseSalesHeader(var SalesHdr: Record "Sales Header";
    Preview: Boolean) LinesWereModified: Boolean
    begin
        PreviewMode := Preview;
    ...
    procedure CalcAndUpdateVATOnLines(var SalesHeader: Record "Sales Header";
    var SalesLine: Record "Sales Line") LinesWereModified: Boolean
    var
    ...
    begin
        ...
        LinesWereModified := SalesLine.UpdateVATOnLines(0, SalesHeader, SalesLine, TempVATAmountLine0) or SalesLine.UpdateVATOnLines(1, SalesHeader, SalesLine, TempVATAmountLine1);
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeCalcInvDiscount(var SalesHeader: Record "Sales Header";
    PreviewMode: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforePerformManualRelease(var SalesHeader: Record "Sales Header";
    PreviewMode: Boolean;
    var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterUpdateSalesDocLines(var SalesHeader: Record "Sales Header";
    var LinesWereModified: Boolean;
    PreviewMode: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCodeOnAfterCheck(SalesHeader: Record "Sales Header";
    var SalesLine: Record "Sales Line";
    var LinesWereModified: Boolean)
    begin
    end;
