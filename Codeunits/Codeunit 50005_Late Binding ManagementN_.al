codeunit 50005 "Late Binding ManagementN"
{
    trigger OnRun()
    begin
    end;
    ...
    begin
        ...
        if TempTrackingSpecification."Buffer Value1" = 0 then exit; // Item tracking is fully allocated => no basis for reallocation
        if not CalcInventory(TempTrackingSpecification) then exit; // No reservations exist => no basis for reallocation
        ...
        if TempTrackingSpecification."Buffer Value4" < 0 then if not PrepareTempDataSet(TempTrackingSpecification, Abs(TempTrackingSpecification."Buffer Value4")) then exit; // There is not sufficient free supply to cover reallocation
        TempTrackingSpecification.Reset;
    ...
    begin
        ...
        if TempTrackingSpecification2.FindSet then
            repeat
                TempCurrSupplyReservEntry.SetTrackingFilter(TempTrackingSpecification2."Serial No.", TempTrackingSpecification2."Lot No.");
    ...
    local procedure PrepareTempDataSet(var TempTrackingSpecification: Record "Tracking Specification" temporary;
    QtyToPrepare: Decimal): Boolean
    var
        ItemLedgEntry: Record "Item Ledger Entry";
        ReservEntry2: Record "Reservation Entry";
    begin
        if QtyToPrepare <= 0 then exit(true);
        TempTrackingSpecification.Reset;
        ...
        if ItemLedgEntry.FindSet then
            repeat
                ...
                if TempTrackingSpecification.IsEmpty then begin
                    ...
                    if TempSupplyReservEntry."Quantity (Base)" = 0 then TempSupplyReservEntry.Delete
                end;
    ...
    local procedure ReshuffleReservEntry(SupplyReservEntry: Record "Reservation Entry";
    QtyToReshuffle: Decimal;
    var TempTrackingSpecification: Record "Tracking Specification" temporary) RemainingQty: Decimal
    var
        ...
        xQty: Decimal;
    begin
        if SupplyReservEntry."Reservation Status" > SupplyReservEntry."Reservation Status"::Tracking then exit; // The entry is neither reservation nor tracking and cannot be reshuffled
        if not SupplyReservEntry.Positive then exit; // The entry is not supply and cannot be reshuffled
    ...
    local procedure MakeConnection(var SupplySurplusEntry: Record "Reservation Entry";
    var DemandReservEntry: Record "Reservation Entry";
    QtyToReshuffle: Decimal) RemainingQty: Decimal
    var
        NewEntryNo: Integer;
    begin
        if SupplySurplusEntry."Quantity (Base)" = 0 then exit(QtyToReshuffle);
    ...
    begin
        ...
        if TempTrackingSpecification.FindSet then
            repeat
                ...
                TempTrackingSpecification."Buffer Value4" := TempTrackingSpecification."Buffer Value2" - // Total Inventory
         ...
         TempTrackingSpecification."Buffer Value1"; // Non-allocated lot/sn demand (signed negatively)
            ...
            until TempTrackingSpecification.Next = 0;
        if TotalReservedQty = 0 then exit(false); // No need to consider reallocation if no reservations exist.
        exit(true);
    end;
    ...
    begin
        ...
        ReservEntry.SetRange("Reservation Status", ReservEntry."Reservation Status"::Reservation, ReservEntry."Reservation Status"::Tracking);
        TempTrackingSpecification.SetFilter("Buffer Value4", '< %1', 0);
        if TempTrackingSpecification.FindSet then
            repeat
                ...
                if ReservEntry.FindSet(true, true) then
                    ...
                    until (ReservEntry.Next = 0) or (TempTrackingSpecification."Buffer Value4" + TempTrackingSpecification."Buffer Value5" >= 0);
    ...
    var
        ...
        ReservEntry2: Record "Reservation Entry";
    begin
        if not ItemLedgEntry.FindSet then exit;
        ReservEntry.SetCurrentKey("Item No.", "Source Type", "Source Subtype", "Reservation Status", "Location Code", "Variant Code");
        ...
        repeat
            ReservEntry2.Get(ReservEntry."Entry No.", not ReservEntry.Positive); // Get demand
            if not ReservEntry2.TrackingExists then UnspecificQty -= ReservEntry2."Quantity (Base)"; // Sum up negative entries to a positive value
        until ReservEntry.Next = 0;
    end;

    procedure ReleaseForReservation(ItemNo: Code[20];
    VariantCode: Code[10];
    LocationCode: Code[10];
    SerialNo: Code[50];
    LotNo: Code[50];
    QtyToRelease: Decimal) AllocationsChanged: Boolean
    var
    ...
    procedure ReleaseForReservation3(var CalcItemLedgEntry: Record "Item Ledger Entry";
    CalcReservEntry: Record "Reservation Entry";
    RemainingQtyToReserve: Decimal) AllocationsChanged: Boolean
    var
    ...
    begin
        ...
        if CalcItemLedgEntry.FindSet then
            repeat
                ...
                AvailableToReserve += CalcItemLedgEntry."Remaining Quantity" - CalcItemLedgEntry."Reserved Quantity";
            until (CalcItemLedgEntry.Next = 0) or (AvailableToReserve >= RemainingQtyToReserve);
        if AvailableToReserve < RemainingQtyToReserve then AllocationsChanged := ReleaseForReservation(CalcReservEntry."Item No.", CalcReservEntry."Variant Code", CalcReservEntry."Location Code", CalcReservEntry."Serial No.", CalcReservEntry."Lot No.", RemainingQtyToReserve - AvailableToReserve);
    end;
    ...
    begin
        ...
        if QtyToReserveBase <= 0 then exit;
        ...
        UnreservedQty := TrackingSpecification."Quantity (Base)" - TrackingSpecification."Quantity Handled (Base)";
        ReservEntry.SetCurrentKey("Source ID", "Source Ref. No.", "Source Type", "Source Subtype", "Source Batch Name", "Source Prod. Order Line", "Reservation Status");
        ...
        if ReservEntry.FindSet then
            ...
            until ReservEntry.Next = 0;
        if QtyToReserveBase > UnreservedQty then QtyToReserveBase := UnreservedQty;
        ReservMgt.AutoReserveOneLine(1, QtyToReserve, QtyToReserveBase, '', AvailabilityDate);
    end;


    procedure ReserveItemTrackingLine2(TrackingSpecification: Record "Tracking Specification";
    QtyToReserve: Decimal;
    QtyToReserveBase: Decimal)
    var
        ReservEntry: Record "Reservation Entry";
